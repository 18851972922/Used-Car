...
public class GaoDuan extends JFrame {// 车辆估价窗体
	...
	private GujiaPanel getGujiaPanel()  {// 初始化loginPanel登录面板的方法
		if (gujiaPanel == null) {// 登录面板中没有组件时
			...
			jLabel.setText("高端车");// 设置“标签的文本内容
			jLabel.setBounds(new Rectangle(50, 70, 100, 18));// 设置标签的位置与宽高
			
			gujiaPanel = new GujiaPanel();// 面板
			gujiaPanel.setLayout(null);// 设置面板的布局为绝对布局
			gujiaPanel.setBackground(new Color(0xD8DDC7));// 设置面板的背景色
			gujiaPanel.add(jLabel, null);// 把标签置于登录面板中
			scpDemo = new JScrollPane();   //滚动面板
			scpDemo.setBounds(new Rectangle(50,100,570,350));
			gujiaPanel.add(scpDemo);
			
			gujiaPanel.add(getBackButton(), null);// 把按钮置于登录面板中
			try {
