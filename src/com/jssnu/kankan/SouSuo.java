...
public class SouSuo extends JFrame {// 车辆估价窗体
	...
	private GujiaPanel getGujiaPanel()  {// 初始化loginPanel登录面板的方法
		if (gujiaPanel == null) {// 登录面板中没有组件时
			...
			jLabel.setText("搜索结果如下：");// 设置“里程”标签的文本内容
			jLabel.setBounds(new Rectangle(50, 70, 150, 18));// 设置“里程”标签的位置与宽高
			...
			try {
				HeadPage ss = new HeadPage();
				BaseDao bd=new BaseDao();
				String sql = "SELECT * FROM cars_message where name like \"%"+ss.getSearchStr()+"%\"";
